...
async function discoverLiveStreamEndpoints(channelId, { proxyUrl = config.dlhdProxyUrl, legacyUrl = config.dlhdUrl } = {}) {
  const rawId = String(channelId || "").replace(/\.(?:m3u8|ts)$/i, "");
  ...
  if (proxyUrl) {
    ...
    try {
      const response = await fetch(`${base}/candidates/${id}`, {
        redirect: "follow",
        signal: AbortSignal.timeout(LIVE_SOURCE_DISCOVERY_TIMEOUT_MS),
      });
      if (response.ok) {
        ...
        for (const row of rows.slice(0, LIVE_SOURCE_MAX_CANDIDATES)) {
          ...
          endpoints.push({
            provider: `daddy:${family}:e${embed + 1}:s${source + 1}`,
            url: `${base}/candidate/${id}/${family}/${embed}/${source}.m3u8`,
            candidate: { family, embed, source },
          });
        }
      }
    } catch {
      // Older/unpatched proxy or transient discovery failure: retain the tested
      // aggregate resolver path rather than making live TV unavailable.
    }
...
async function resolveLiveEndpoint(endpoint) {
  ...
  try {
    ...
    throw new LiveEndpointError(`${endpoint.provider} transport failed`, { transport: true, provider: endpoint.provider });
  }
  ...
  if (!text.trimStart().startsWith("#EXTM3U")) {
    throw new LiveEndpointError(`${endpoint.provider} returned no HLS manifest`, { status: response.status, invalid: true, provider: endpoint.provider });
  }
  return endpoint.url;
}
...
export function retainLiveSourceLearning(channelId, { proxyUrl = config.dlhdProxyUrl, legacyUrl = config.dlhdUrl } = {}) {
  ...
  if (!session) {
    session = { refs: 0, releaseManager: null, cancelled: false };
    learningSessions.set(id, session);
    void discoverLiveStreamEndpoints(id, { proxyUrl, legacyUrl })
      .then((endpoints) => {
  ...
  return () => {
    ...
    session.releaseManager?.();
    learningSessions.delete(id);
  };
}
